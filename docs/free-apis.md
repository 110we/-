...
本文档整理自开源项目 [public-apis/public-apis](https://github.com/public-apis/public-apis)（2026-08 抓取），收录了涵盖数十个领域的免费公开 API。表格字段说明：

- **API**：接口名称（链接为官方文档/主页）
- **Description**：功能说明
- **Auth**：认证方式（`No`=无需认证；`apiKey`=注册免费 key；`OAuth`=OAuth 授权）
- **HTTPS**：是否支持 HTTPS
- **CORS**：是否支持跨域

> 提示：优先选用 Auth 为 `No` 的接口，无需注册即可直接调用；`apiKey` 接口多为免费注册后获取 key 使用。
